...
def conv_r(in_f, out_f, kernel_one, kernel_two,stride=1, bias=True, pad='zero', downsample_mode='stride'):
    downsampler = None
    if stride != 1 and downsample_mode != 'stride':
    ...
    if pad == 'reflection':
        padder = nn.ReplicationPad3d(to_pad)
        to_pad = 0
    convolver_one = nn.Conv3d(in_f, out_f, kernel_one, stride, padding=to_pad, bias=bias)
    convolver_two = nn.Conv3d(out_f, out_f, kernel_two, 1, padding=0, bias=bias)

    layers = filter(lambda x: x is not None, [padder, convolver_one, convolver_two, downsampler])
    return nn.Sequential(*layers)